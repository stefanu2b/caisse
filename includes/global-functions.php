...
if ( ! function_exists( 'ddwcpos_form_field' ) ) {
	/**
	 * Display Form field function
	 *
	 * @return void
	 */
	function ddwcpos_form_field( $args ) {
		$defaults = [
			'type'                  => 'text',
			'name'                  => '',
			'value'                 => '',
		...
		$args = apply_filters( 'ddwcpos_form_field_args', $args, $name, $value );
		...
		if ( $args['required'] ) {
			...
			$required        = '&nbsp;<abbr class="required" title="' . esc_attr__( 'required', 'ddwc-multipos' ) . '">*</abbr>';
		} else {
			$required = '';
		}
		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<select id="billing_country" name="' . esc_attr( $name ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( $default_input_class ) . ' ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_attr__( 'Select a country / region&hellip;', 'ddwc-multipos' ) ) . '" ' . $data_label . '><option value="">' . esc_html__( 'Select a country / region&hellip;', 'ddwc-multipos' ) . '</option>';
					...
					$field .= '<noscript><button type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country / region', 'ddwc-multipos' ) . '">' . esc_html__( 'Update country / region', 'ddwc-multipos' ) . '</button></noscript>';
				}
			...
			case 'state':
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select id="billing_state" name="' . esc_attr( $name ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( $default_input_class ) . ' ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_html__( 'Select an option&hellip;', 'ddwc-multipos' ) ) . '"  data-input-classes="' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . $data_label . '>
						<option value="">' . esc_html__( 'Select an option&hellip;', 'ddwc-multipos' ) . '</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( '' === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : esc_html__( 'Choose an option', 'ddwc-multipos' );
							}
		...
		if ( ! empty( $field ) ) {
			...
			if ( $args['description'] ) {
				if ( $is_admin ) {
					$field_html .= '<p class="description '. ( $args[ 'no_description_margin' ] ? '' : 'ddwcpos-margin-left-20' ) .'" id="' . esc_attr( $args['id'] ) . '-description" aria-hidden="true"><i>' . wp_kses_post( $args['description'] ) . '</i></p>';
				} else {
		 ...
		 * Filter by type.
		 */
		$field = apply_filters( 'ddwcpos_form_field_' . $args['type'], $field, $name, $args, $value );

		/**
		 * General filter on form fields.
		 */
		$field = apply_filters( 'ddwcpos_form_field', $field, $name, $args, $value );
